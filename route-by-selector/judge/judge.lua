...
local condition = require "kong.plugins.route-by-selector.judge.condition"
...
function _M.filter_complicated_conditions(expression, conditions)
    ...
    pass = func()
    if pass then
        ngx.log(ngx.INFO, "filter_complicated_conditions: ", expression)
    end
...
function _M.judge_rule(rule)
    if not rule or not rule.conditions then 
        return false
    end

    local judge_type = rule.type
    ...
    elseif judge_type == 3 then 
        pass = _M.filter_complicated_conditions(judge.expression, conditions)
    end
